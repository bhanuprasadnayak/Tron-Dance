...
bool wipeRightSideOut(CRGB c, uint32_t duration) {
  int m = SHIRT_RIGHT_LEDS + HAND_RIGHT_LEDS;
  int s = duration / m;
  for (int i = 1; i <= m; i++) {
     if (i <= SHIRT_RIGHT_LEDS) shirt_right.setLed(i, c); 
     else hand_right.setLed(i - SHIRT_RIGHT_LEDS, c);
     shirt_right.show(); hand_right.show(); if (safeDelay(s)) return true;
  }
  return false;
}
...
void runChoreography() {
  Serial.println("Starting Suit 1 Final...");

  // 0:00 - Start Off
  clear_all();
  if (safeDelay(12000)) return; 

  // 0:12 - Hands Only On [Dark Blue]
  ...
  // 0:34 - MULTICOLOUR [Rainbow Fall]
  // Corrected: Multicolor from 0:34 to 0:42
  uint32_t rbTime = 0;
  while(rbTime < 8000) { if (rainbow_fall(10, 5)) return; rbTime += 10; }

  // 0:42 - RANDOM
  // Corrected: Random from 0:42 to 0:47
  ...
  // 1:11 - Right hand + Right jacket [Dark Green]
  if (wipeRightSideOut(CRGB::DarkGreen, 1000)) return;
  ...
  // 1:37 - All on top to bottom [Purple pant/hat, Dk Green jacket/hand]
  ...
  // 1:55 - All on (White) fade out bottom
  set_static_all(CRGB::White);
  if (safeDelay(1000)) return; 
  ...
  // 2:19 - Rainbow Fall (Long Gap)
  rbTime = 0;
  while(rbTime < 19000) { if (rainbow_fall(10, 5)) return; rbTime += 10; }

  // 2:38 - All off top to bottom
  ...
  // 3:26 to 3:38 (Gap - Skip Mustard Yellow)
  if (safeDelay(12000)) return; 
  ...
  // 6:06 - Fade in bottom to top [Red]
  ...
  Serial.println("Suit 1 Choreography Done.");
  MODE = 2;
}
