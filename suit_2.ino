...
bool wipeLeftSideOut(CRGB c, uint32_t duration) {
  int maxLeft = SHIRT_LEFT_LEDS + HAND_LEFT_LEDS;
  int stepDelay = duration / maxLeft;
  for (int i = 1; i <= maxLeft; i++) {
     if (i <= SHIRT_LEFT_LEDS) shirt_left.setLed(i, c);
     else hand_left.setLed(i - SHIRT_LEFT_LEDS, c);
     shirt_left.show(); hand_left.show();
     if (safeDelay(stepDelay)) return true;
  }
  return false;
}
...
void runChoreography() {
  Serial.println("Starting Suit 2 Final...");

  // 0:00 - All off
  clear_all();
  if (safeDelay(11000)) return; 

  // 0:11 - Hands only on [Dark Blue]
  ...
  // 0:15 - Left hand on [Dark Blue]
  hand_left.setStatic(1, HAND_LEFT_LEDS, CRGB::DarkBlue);
  if (safeDelay(2660)) return; 

  // 0:17:66 - Right hand on [Dark Blue]
  hand_right.setStatic(1, HAND_RIGHT_LEDS, CRGB::DarkBlue);
  if (safeDelay(340)) return; 
  ...
  // 0:36 - MULTICOLOUR [Rainbow Fall]
  // PDF Suit 2: 0:36
  uint32_t rbTime = 0;
  while(rbTime < 6000) { if (rainbow_fall(10, 5)) return; rbTime += 10; }
  ...
  // 1:13 - Left hand + Left jacket (Fade in) [Dark Green]
  // Corrected to start at 1:13 (PDF)
  if (wipeLeftSideOut(CRGB::DarkGreen, 1000)) return;
  // Hold until 1:27.5 (approx 13.5s)
  if (safeDelay(13500)) return; 

  // 1:27:5 - Off All
  clear_all();
  if (safeDelay(9500)) return; 

  // 1:37 - All on top to bottom [Purple/Dark Green]
  ...
  // Wait until 2:18.5
  if (safeDelay(6500)) return;
  clear_all();
  if (safeDelay(500)) return;

  // 2:19 - Rainbow Fall
  rbTime = 0;
  while(rbTime < 16000) { if (rainbow_fall(10, 5)) return; rbTime += 10; }
  ...
  // 3:31 - MUSTARD YELLOW (Specific to Suit 2)
  set_static_all(CRGB::Gold);
  if (safeDelay(5000)) return; 
  clear_all();
  if (safeDelay(10000)) return; 

  // 3:46 - On/Off [Light Blue]
  set_static_all(CRGB::LightBlue);
  if (safeDelay(9000)) return; 
  clear_all();
  if (safeDelay(1000)) return; 

  // 3:56 - On/Off [Light Green]
  set_static_all(CRGB::LightGreen);
  if (safeDelay(3000)) return; 
  clear_all();
  if (safeDelay(16000)) return; 

  // 4:15 - On/Off [Light Green]
  set_static_all(CRGB::LightGreen);
  if (safeDelay(4000)) return; 
  clear_all();
  if (safeDelay(11000)) return; 
  ...
  if (safeDelay(4000)) return; 
  clear_all();
  if (safeDelay(35000)) return; 

  // 5:09 - Final On [Pink]
  set_static_all(CRGB::DeepPink);
  if (safeDelay(32000)) return; 
  clear_all();
  if (safeDelay(25000)) return;
  
  // 6:06 - Fade In [Red]
  if (wipeBottomUp(CRGB::Red, 2000)) return;
  if (safeDelay(22000)) return; 
  clear_all();
  if (safeDelay(10000)) return; 
  ...
  Serial.println("Suit 2 Choreography Done.");
  MODE = 2;
}
